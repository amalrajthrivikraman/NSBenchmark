...
public class SearchItems extends DriverSetup {
	...
	final static Logger logger = Logger.getLogger(com.nscomp.tests.SearchItems.class);
	...
	public void xsearchOnCategory(){
		...
		String testCaseName = "TC03 - Search on Category";
		...
		try{
			...
			childTest = ExtentReportsUtil.startTest("Step_2: Choose Garden & Outdoors");
			logger.info("Step_2: Choose Garden & Outdoors");
			amazonHome.selectCategory("Garden & Outdoors");
			childTest.log(LogStatus.PASS, "Search Category Selected as XXX");
			extTestlogger.appendChild(childTest);
			...
			Boolean result = amazonHome.verifySearchItem(itemName, price);
			if(result == true){
